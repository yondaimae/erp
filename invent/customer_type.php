...
<?php
			$sCode 	= isset( $_POST['sCode'] ) ? trim( $_POST['sCode'] ) : ( getCookie('sTypeCode') ? getCookie('sTypeCode') : '' );
			$sName	= isset( $_POST['sName'] ) ? trim( $_POST['sName'] ) : ( getCookie('sTypeName') ? getCookie('sTypeName') : '' );
?>
...
<?php
	...
	if( $sCode != '' )
	{
		createCookie('sTypeCode', $sCode);
		$where .= "AND code LIKE '%". $sCode ."%' ";
	}

	if( $sName != '' )
	{
		createCookie('sTypeName', $sName);
	...
	$get_rows	= get_rows();
	$paginator->Per_Page('tbl_customer_type', $where, $get_rows);
	$paginator->display($get_rows, 'index.php?content=customer_type');

	$qs = dbQuery("SELECT * FROM tbl_customer_type ".$where." LIMIT ".$paginator->Page_Start.", ".$paginator->Per_Page);
?>
	<div class="row">
    	<div class="col-sm-12">
		...
		<div class="modal-dialog" style="width:500px;">
			<div class="modal-content">
	  			<div class="modal-header">
					...
					<h4 class="modal-title" id="modal_title">เพิ่มชนิดลูกค้า</h4>
				 </div>
				 <div class="modal-body" id="modal_body">
                 <div class="row">
                 	<div class="col-sm-6">
                    	<label>รหัส</label>
                        <input type="text" class="form-control input-sm" id="addCode" placeholder="กำหนดรหัสชนิดลูกค้า" />
                        <span class="help-block red" id="addCode-error"></span>
                    </div>
                    <div class="divider-hidden"></div>
                    <div class="col-sm-12">
                    	<label>ชื่อชนิดลูกค้า</label>
                        <input type="text" class="form-control input-sm" id="addName" placeholder="กำหนดชื่อชนิดลูกค้า" />
		...
		<div class="modal-dialog" style="width:500px;">
			<div class="modal-content">
                 ...
                 <div class="row">
                 	<div class="col-sm-6">
                    	<label>รหัส</label>
                        <input type="text" class="form-control input-sm" id="editCode" placeholder="กำหนดรหัสชนิดลูกค้า" />
                        <span class="help-block red" id="editCode-error"></span>
                    </div>
                    <div class="divider-hidden"></div>
                    <div class="col-sm-12">
                    	<label>ชื่อชนิดลูกค้า</label>
                        <input type="text" class="form-control input-sm" id="editName" placeholder="กำหนดชื่อชนิดลูกค้า" />
...
<script src="script/customer_type.js"></script>
