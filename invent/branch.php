...
<div class="row top-row">
	<div class="col-sm-6 top-col">
    	<h4 class="title"><i class="fa fa-bank"></i>&nbsp;<?php echo $pageTitle; ?></h4>
    </div>
    <div class="col-sm-6">
...
<?php
			$sCode 	= isset( $_POST['sCode'] ) ? trim( $_POST['sCode'] ) : ( getCookie('sBranchCode') ? getCookie('sBranchCode') : '' );
			$sName	= isset( $_POST['sName'] ) ? trim( $_POST['sName'] ) : ( getCookie('sBranchName') ? getCookie('sBranchName') : '' );
?>
...
<?php
	...
	if( $sCode != '' )
	{
		createCookie('sBranchCode', $sCode);
		$where .= "AND code LIKE '%". $sCode ."%' ";
	}

	if( $sName != '' )
	{
		createCookie('sBranchName', $sName);
	...
	$get_rows	= get_rows();
	$paginator->Per_Page('tbl_branch', $where, $get_rows);
	$paginator->display($get_rows, 'index.php?content=branch');

	$qs = dbQuery("SELECT * FROM tbl_branch ".$where." LIMIT ".$paginator->Page_Start.", ".$paginator->Per_Page);
?>
	<div class="row">
    	<div class="col-sm-12">
