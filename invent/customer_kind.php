...
<?php
			$sCode 	= isset( $_POST['sCode'] ) ? trim( $_POST['sCode'] ) : ( getCookie('sKindCode') ? getCookie('sKindCode') : '' );
			$sName	= isset( $_POST['sName'] ) ? trim( $_POST['sName'] ) : ( getCookie('sKindName') ? getCookie('sKindName') : '' );
?>
...
<?php
	...
	if( $sCode != '' )
	{
		createCookie('sKindCode', $sCode);
		$where .= "AND code LIKE '%". $sCode ."%' ";
	}

	if( $sName != '' )
	{
		createCookie('sKindName', $sName);
	...
	$get_rows	= get_rows();
	$paginator->Per_Page('tbl_customer_kind', $where, $get_rows);
	$paginator->display($get_rows, 'index.php?content=customer_kind');

	$qs = dbQuery("SELECT * FROM tbl_customer_kind ".$where." LIMIT ".$paginator->Page_Start.", ".$paginator->Per_Page);
?>
	<div class="row">
    	<div class="col-sm-12">
...
<script src="script/customer_kind.js"></script>
