<?php 
	...
	elseif( isset($_GET['add']) && isset($_GET['id_return_sponsor'])) :
		$btn .= "<a href='index.php?content=sponsor_return' style='text-decoration:none;'><button type='button' class='btn btn-warning'><i class='fa fa-arrow-left'></i>&nbsp; กลับ</button></a>";
...
<div class="row">
	<div class="col-xs-6">
    	<h3 class="title"><span class="glyphicon glyphicon-import"></span>&nbsp;<?php echo $page_name; ?></h3>
	</div>
    <div class="col-xs-6">
       <p class="pull-right">
       	<?php echo $btn; ?>
...
<!------------------------------------------------  เพิ่มเอกสารใหม่  --------------------------------->
		<!-----------------  หัวเอกสาร  ------------------->
<form id="add_form" action="controller/returnController.php?add=y&role=4" method="post">
<div class="row">
...
?>
		<tr id="row<?php echo $rs['id_return_sponsor_detail']; ?>">
        	<td><?php echo thaiDateTime($rs['date_add']); ?></td>
            <td id="product<?php echo $rs['id_return_sponsor_detail']; ?>"><?php echo $product_name; ?></td>
            <td align="right"><?php echo number_format($rs['qty']); ?><input type="hidden" class='qty' id="qty<?php echo $rs['id_return_sponsor_detail'];?>" value="<?php echo $rs['qty']; ?>"/></td>
            <td id="zone<?php echo $rs['id_return_sponsor_detail']; ?>" align="center"><?php echo get_zone($rs['id_zone']); ?></td>
            ...
            <td align="right"><button type="button" class="btn btn-danger btn-xs btn-block" onclick="delete_row(<?php echo $rs['id_return_sponsor_detail']; ?>)"><i class="fa fa-trash"></i></button></td>
        </tr>
<?php  	endwhile;	?>
<?php else : ?>
...
?>
		<tr id="row<?php echo $rs['id_return_sponsor_detail']; ?>">
            ...
            <td align="center"><?php echo get_warehouse_name_by_id(get_warehouse_by_zone($rs['id_zone'])); ?></td>
            <td align="center"><?php echo isActived($rs['status']); ?></td>
            <td align="right"><?php if($edit) : ?><button type="button" class="btn btn-danger btn-xs btn-block" onclick="delete_row(<?php echo $rs['id_return_sponsor_detail']; ?>)"><i class="fa fa-trash"></i></button><?php endif; ?></td>
        </tr>
<?php  	endwhile;	?>
<?php else : ?>
		<tr><td colspan="7" align="center"><h4>---------- ไม่มีรายการ  ----------</h4></td></tr>
<?php endif; ?>
	...
	<?php else : /// if(isset($_GET['id_return_sponsor']) ?>
...
<?php
		...
		$where = "WHERE id_return_sponsor != 0 ";
		if( $text != "" ) :
			switch( $filter) :
				case "customer" :
				$in_cause = "";
...
$paginator = new paginator();
if(isset($_POST['get_rows'])){$get_rows = $_POST['get_rows'];$paginator->setcookie_rows($get_rows);}else if(isset($_COOKIE['get_rows'])){$get_rows = $_COOKIE['get_rows'];}else{$get_rows = 50;}
		$paginator->Per_Page("tbl_return_sponsor",$where,$get_rows);
...
<?php $sql = dbQuery("SELECT tbl_customer.id_customer, first_name, last_name, tbl_sponsor.id_sponsor, id_sponsor_budget, balance, start, end FROM tbl_customer JOIN tbl_sponsor ON tbl_customer.id_customer = tbl_sponsor.id_customer JOIN tbl_sponsor_budget ON tbl_sponsor.id_sponsor = tbl_sponsor_budget.id_sponsor AND tbl_sponsor.year = tbl_sponsor_budget.year WHERE tbl_sponsor.active = 1 AND tbl_sponsor_budget.active =1"); ?>
<?php if(dbNumRows($sql) > 0 ) : ?>
        	<script>
			var CustomerList = [
        	<?php while($rs = dbFetchArray($sql)) : ?>
            	...
            	<?php echo "\"".$rs['id_customer']." : ".$rs['first_name']." ".$rs['last_name']." : "."คงเหลือ"." : ".$rs['balance']."\","; ?>
				<?php endif; ?>
            <?php endwhile; ?>
			];
			</script>
...
function add()
{
	...
	{
		$.ajax({
			url:"controller/returnController.php?check_sponsor_order",
			type:"POST", cache:false, data: {"reference" : order_reference, "id_customer" : id_customer},
			success: function(rs)
			{
				if(rs == "ok")
				{ 
					...
					load_out();
					swal("รหัสอ้างอิงไม่ตรงกับชื่อลูกค้า", "เลขที่บิลอาจผิด หรือ ลูกค้ารายนี้ไม่มีส่วนเกี่ยวข้องกับเลขที่บิลนี้", "error");
				}
			}
		});
	}else{
...
function update(id)
{
	...
	$.ajax({
		url:"controller/returnController.php?check_sponsor_order",type:"POST", cache:false,
		data:{ "id_customer" : id_customer, "reference" : reference },
		success: function(rs)
		{
			if(rs == "ok")
			{
				$.ajax({
					...
					data:{ "id_return_sponsor" : id, "date_add" : date, "id_customer" : id_customer, "reference" : reference, "remark" : remark },
					success: function(ra)
					{
						if(ra == "success")
						{
...
{
	load_in();
	var id_return_sponsor 	= $("#id_return_sponsor").val();
	...
	if( id_zone == "")
	{
		...
		$.ajax({
			...
			data: {"id_return_sponsor" : id_return_sponsor, "id_zone" : id_zone, "barcode" : barcode, "qty" : qty },
...
function sum_item()
{
	...
	var id_return_sponsor 	= $("#id_return_sponsor").val();
	$.ajax({
		url: "controller/returnController.php?sum_item&id_return_sponsor="+id_return_sponsor, type:"POST", cache:false,
		success: function(data)
		{
			if(data == "fail" )
			{
...
function delete_row(id)
{
	...
	swal({
	  ...
	  if (isConfirm) {
		  load_in();
		$.ajax({
			url:"controller/returnController.php?delete_row&id_return_sponsor_detail="+id, type:"GET", cache:false,
			success: function(rs)
			{
				if(rs == "success")
				{
...
function delete_return(id, reference)
{
	...
	swal({
	  ...
	  if (isConfirm) {
		  load_in();
		$.ajax({
			url:"controller/returnController.php?delete_return&id_return_sponsor="+id, type:"GET", cache:false,
			success: function(rs)
			{
				if(rs == "success")
				{
...
function save_add()
{
	...
	var id_return_sponsor = $("#id_return_sponsor").val();
	$.ajax({
		url:"controller/returnController.php?save_add&id_return_sponsor="+id_return_sponsor,
...
function save_edit()
{
	...
	var id_return_sponsor = $("#id_return_sponsor").val();
	$.ajax({
		url:"controller/returnController.php?save_edit&id_return_sponsor="+id_return_sponsor,
		type:"GET", cache:false,
		success: function(rs)
		{
			var rs = $.trim(rs);
