...
$qr .= "FROM tbl_sponsor_budget AS bd ";
$qr .= "LEFT JOIN tbl_sponsor AS sp ON bd.id_sponsor = sp.id ";
$qr .= "JOIN tbl_customer AS cs ON sp.id_customer = cs.id ";
$qr .= "WHERE bd.id_sponsor != 0 ";
...
//------- Report name Row 1
$excel->getActiveSheet()->setCellValue('A1', 'รายงานสรุปยอดสปอนเซอร์แยกตามผู้รับ ณ วันที่ ' . date('d/m/Y'));
...
setToken($_GET['token']);
$file_name = "Sponsor Summary.xlsx";
