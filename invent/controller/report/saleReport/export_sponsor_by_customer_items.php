...
$qr  = "SELECT product_code, product_name, customer_code, customer_name, SUM(qty) AS qty, SUM(total_amount_inc) AS amount ";
...
//------- Report name Row 1
$excel->getActiveSheet()->setCellValue('A1', 'รายงานสรุปยอดสปอนเซอร์แยกตามผู้รับ แสดงเลขที่รายการสินค้า ณ วันที่ ' . thaiDate($fromDate,'/') .' ถึง '.thaiDate($toDate, '/'));
...
$excel->getActiveSheet()->setCellValue('B4', 'รหัส');
$excel->getActiveSheet()->setCellValue('C4', 'ผู้รับ');
$excel->getActiveSheet()->setCellValue('D4', 'รหัสสินค้า');
$excel->getActiveSheet()->setCellValue('E4', 'ชื่อสินค้า');
...
if(dbNumRows($qs) > 0)
{
  ...
  while($rs = dbFetchObject($qs))
  {
    ...
    $excel->getActiveSheet()->setCellValue('B'.$row, $rs->customer_code);
    $excel->getActiveSheet()->setCellValue('C'.$row, $rs->customer_name);
...
setToken($_GET['token']);
$file_name = "SponsoredByItems.xlsx";
