...
<?php
			$sCode 	= isset( $_POST['sCode'] ) ? trim( $_POST['sCode'] ) : ( getCookie('sCategoryCode') ? getCookie('sCategoryCode') : '' );
			$sName	= isset( $_POST['sName'] ) ? trim( $_POST['sName'] ) : ( getCookie('sCategoryName') ? getCookie('sCategoryName') : '' );
?>
...
<?php
	...
	if( $sCode != '' )
	{
		createCookie('sCategoryCode', $sCode);
		$where .= "AND code LIKE '%". $sCode ."%' ";
	}

	if( $sName != '' )
	{
		createCookie('sCategoryName', $sName);
	...
	$get_rows	= get_rows();
	$paginator->Per_Page('tbl_product_category', $where, $get_rows);
	$paginator->display($get_rows, 'index.php?content=category');

	$qs = dbQuery("SELECT * FROM tbl_product_category ".$where." LIMIT ".$paginator->Page_Start.", ".$paginator->Per_Page);
?>
	<div class="row">
    	<div class="col-sm-12">
