...
function getEdit(id){
	$.ajax({
		url:"controller/subGroupController.php?getData",
		type:"GET", cache:"false", data:{ "id" : id },
		success: function(rs){
			var rs = $.trim(rs);
			var arr = rs.split(' | ');
			if( arr.length == 3 ){
				$("#id_sub_group").val( arr[0] );
				$("#editCode").val( arr[1] );
				$("#editName").val( arr[2] );
...
function saveEdit(){
	...
	$.ajax({
		url:"controller/subGroupController.php?saveEdit",
		type:"POST", cache:"false", data:{ "id" : id, "code" : code, "name" : name },
		success: function(rs){
			var rs = $.trim(rs);
			if( rs == 'success' ){
				$("#edit-modal").modal('hide');
				swal({ title: "Updated", text: "บันทึกข้อมูลเรียบร้อยแล้ว", type: "success", timer: 1000 });
...
function addNew(){
	...
	$.ajax({
		url:"controller/subGroupController.php?addNew",
		type:"POST", cache:"false", data:{ "code" : code, "name" : name },
		success: function(rs){
			var rs = $.trim(rs);
			if( rs == 'success' ){
				$("#add-modal").modal('hide');
				swal({ title: "Success", text: "เพิ่มข้อมูลเรียบร้อยแล้ว", type: "success", timer: 1000 });
				setTimeout(function(){ window.location.reload(); }, 1200);
			}else if( rs == 'nameError' ){
...
function remove(id, code){
	swal({
		...
		$.ajax({
			url:"controller/subGroupController.php?deleteSubGroup",
			type:"GET", cache:"false", data: { "id" : id },
			success: function(rs){
				var rs = $.trim(rs);
				if( rs == 'success' ){
...
function goBack(){
	window.location.href = "index.php?content=product_sub_group";
}
...
function clearFilter(){
	$.get("controller/subGroupController.php?clearFilter", function(){ goBack(); });
}
