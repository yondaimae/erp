...
function getEdit(id){
	$.ajax({
		url:"controller/kindController.php?getData",
...
function saveEdit(){
	...
	$.ajax({
		url:"controller/kindController.php?saveEditKind",
...
function addNew(){
	...
	$.ajax({
		url:"controller/kindController.php?addKind",
...
function remove(id, code){
	swal({
		...
		$.ajax({
			url:"controller/kindController.php?deleteKind",
			type:"GET", cache:"false", data: { "id" : id },
			success: function(rs){
				var rs = $.trim(rs);
				if( rs == 'success' ){
...
function goBack(){
	window.location.href = "index.php?content=kind";
}
...
function clearFilter(){
	$.get("controller/kindController.php?clearFilter", function(){ goBack(); });
}
