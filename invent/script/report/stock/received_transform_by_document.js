function getReport(){
  ...
  load_in();
  $.ajax({
    url:'controller/stockReportController.php?received_transform_by_document',
    type:'GET',
    cache:'false',
    data:{
      'report' : 'Y',
...
function doExport(){
  ...
  var target = 'controller/stockReportController.php?received_transform_by_document&export';
  target += '&'+data;
  target += '&token='+token;
  get_download(token);
